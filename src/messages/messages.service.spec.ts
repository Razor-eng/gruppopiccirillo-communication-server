...
describe('MessagesService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw error for duplicate attachment field', async () => {
      const dto: CreateMessageDto = {
        conversation_id: '507f1f77bcf86cd799439015',
        content: 'Hello',
        direction: Direction.incoming,
      ...
      mockPrismaService.attachment.create.mockRejectedValue({
        code: 'P2002',
        meta: { target: ['url'] },
      });
      await expect(service.create(dto)).rejects.toThrow('Duplicate url found');
    });
  });
  ...
  describe('findAll', () => {
    it('should return all active messages', async () => {
      ...
      const result = await service.findAll(undefined, { skip: 0, take: 10 });
